...
export function meta({ loaderData }: Route.MetaArgs) {
  return [
    {
      title: loaderData.isEditorRoute ? `Edit: ${loaderData.path}` : loaderData.data.root?.props?.title,
    },
  ];
}
...
export default function PuckSplatRoute({ loaderData }: Route.ComponentProps) {
  return <div>{loaderData.isEditorRoute ? <Editor /> : <Render config={config} data={loaderData.data} />}</div>;
}
